...
function cmbAddSection(wrapperLocation, htmlDom, identifier, type, parentIdentifier) {
    ...
    cmbRefactorLangIdentifier(lastAddedElement);
...
function cmbLangCountingRearrange(){
    let id = false;
    let rand = Math.floor(Math.random() * 100);
    if(cmbNonExistingLangIdArray.length>0){
        id = cmbNonExistingLangIdArray[0];
        delete cmbNonExistingLangIdArray[0];
        if(cmbNonExistingLangIdArray.length>0) {
            cmbNonExistingLangIdArray = cmbNumericSort(cmbNonExistingLangIdArray);
        }
    }
    return Date.now() + rand + id++;
}
...
function cmbRefactorLangIdentifier(lastAddedElement) {
    lastAddedElement.find('.cmb-editable-text-container').each(function(){
        let id = cmbLangCountingRearrange();
        id = id ? id : cmbCountedExistingLangElements;
        $(this).attr('data-cmb-lang-id', id);
        currentLangData[id] = $(this).html();
        if(id==cmbCountedExistingLangElements){
            cmbCountedExistingLangElements++;
        }
    });
    lastAddedElement.find('.cmb-single-line-editable-text').each(function(){
        let id = cmbLangCountingRearrange();
        id = id ? id : cmbCountedExistingLangElements;
        $(this).attr('data-cmb-lang-id', id);
        currentLangData[id] = $(this).html();
...
function cmbSaveStyle(style,htmlText) {
    $.ajax({
        ...
        data: {
            ...
            style: style,
            body: htmlText,
            currentLangData:  JSON.stringify(currentLangData)
        },
