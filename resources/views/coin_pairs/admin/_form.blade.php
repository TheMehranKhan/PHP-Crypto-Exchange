...
{{--is_futures--}}
<div class="form-group my-3 {{ $errors->has('is_futures') ? 'has-error' : '' }}">
    <label for="is_active" class="control-label required">{{ __('Active Futures') }}</label>
    <div>
        <div class="lf-switch">
            {{ Form::radio('is_futures', ACTIVE, true, ['id' => 'is_futures-active', 'class' => 'lf-switch-input']) }}
            <label for="is_futures-active" class="lf-switch-label lf-switch-label-on">{{ __('Active') }}</label>

            {{ Form::radio('is_futures', INACTIVE, false, ['id' => 'is_futures-inactive', 'class' => 'lf-switch-input']) }}
            <label for="is_futures-inactive" class="lf-switch-label lf-switch-label-off">{{ __('Inactive') }}</label>
        </div>
        <span class="invalid-feedback" data-name="is_futures">{{ $errors->first('is_active_futures') }}</span>
    </div>
</div>
