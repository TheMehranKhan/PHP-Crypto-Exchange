...
<div v-else>
    {{--address--}}
    <div v-if="showAddress" class="form-group {{ $errors->has('address') ? 'has-error' : '' }}">
        <label for="address"
               class="control-label required">{{ __('Bank Account') }}</label>
        <div>
    ...
    <div v-else>
        {{ Form::hidden('address', 'ADVCash', ['class'=>'form-control lf-toggle-bg-input lf-toggle-border-color', 'id' => 'address']) }}
    </div>
</div>
