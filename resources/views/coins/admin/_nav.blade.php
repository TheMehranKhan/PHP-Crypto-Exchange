<ul class="lf-nav-tab lf-toggle-border-color border-top border-right border-left">
    ...
    <a class="nav-link {{ is_current_route('coins.bonus.edit','active') }}"
       href="{{ route('coins.bonus.edit', $coin->symbol) }}">{{ __('Bonus') }}</a>
    <a class="nav-link {{ is_current_route('coins.stake.edit','active') }}"
       href="{{ route('coins.stake.edit', $coin->symbol) }}">{{ __('Stake') }}</a>
</ul>
