...
{{--Token-Info--}}
<div class="form-group my-3">
    <label for="token_info" class="control-label required">{{ __('Token Info') }}
        <small class="text-warning d-block">{{ __('Information about this coin; It will be sho in Exchange and Futures Pages.') }}</small>
    </label>
    {{ Form::text('token_info',  null, ['class'=> form_validation($errors, 'token_info'), 'id' => 'token_info',
    'placeholder' => __('ex: https://coinmarketcap.com/currencies/bitcoin')]) }}
    <span class="invalid-feedback" data-decimal_place="token_info">{{ $errors->first('token_info') }}</span>
</div>
...
<div v-if="showERC20Fields">
    ...
    <div class="form-group my-3">
        ...
        'placeholder' => __('ex: 0xD653D79b694EbB9De42Ee10f0AAA610615De96E7'),"data-contract_address"=>"1"]) }}
        <span class="invalid-feedback"
              data-contract_address="contract_address">{{ $errors->first('contract_address') }}</span>
    </div>
...
<div class="form-group my-3">
    ...
    <div>
        <div class="lf-switch">
            ...
            <label for="exchange_status-inactive"
                   class="lf-switch-label lf-switch-label-off">{{ __('Disable') }}</label>
        </div>
