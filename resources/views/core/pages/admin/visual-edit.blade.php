@extends('layouts.master', ['hideBreadcrumb' => $visualPage->settings['hide_breadcrumb'], 'cmbPage' => $visualPage, 'directionMode'=>language(config('app.fallback_locale'))['direction']])

@section('content')
    <div class="cmb-content-wrapper" data-name="Main Wrapper" id="cmb-content-wrapper">
        {{ view_html(short_code(dynamic_language($visualPage->body,$fallbackLang))) }}
    </div>
    <div id="cmb-admin-controller">
        ...
        <a class="" href="javascript:" id="cmb-admin-regenerate-lang">{{ __('Reset Lang') }}<i class="fa fa-spinner float-right ml-1"></i></a>
        ...
        <div class="centralize-wrapper">
            <div class="centralize-inner">
                <div class="centralize-content bg-white text-center">
                    <div class="font-size-50 text-warning">
                        <i class="fa fa-question-circle"></i>
                    </div>
                    <div class="text-muted">
                        {{__('Do you want to continue with language regeneration? It will reset all previous language for this page.')}}
                    </div>
                    <div class="mt-4 mb-2">
                        <a href="javascript:;" class="btn btn-sm btn-success" id="continue-lang-regen">{{__('Continue')}}</a>
                        <a href="javascript:;" class="btn btn-sm btn-danger" id="cancel-lang-regen">{{__('Cancel')}}</a>
                    </div>
                </div>
            </div>
        </div>
...
@section('script')
    <script>
        ...
        let savePageUrl = '{{ route('admin.pages.visual-update', $visualPage->id) }}';
        let dynamicContentUrl = '{{ route('admin.dynamic-content') }}';
        let currentLangData = {{view_html($currentLangText)}};
    ...
    <script type="text/javascript">
        (function($) {
            ...
            $('#cancel-lang-regen').on('click',function(){
                $('.lang-confirmation-modal').addClass('d-none')
            })
            $('#cmb-admin-regenerate-lang').on('click',function(){
                $('.lang-confirmation-modal').removeClass('d-none')
            })
            $('#continue-lang-regen').on('click',function(){
                if(!cmbWorkOnProcess){
                    $('.lang-confirmation-modal').addClass('d-none');
                    $('#cmb-loading-queue').removeClass('d-none');
                    cmbWorkOnProcess = true;
                    cmbCountedExistingLangElements = 1;
                    cmbNonExistingLangIdArray = [];
                    currentLangData = {};
                    let singleLineItems = $('#cmb-content-wrapper').find('.cmb-single-line-editable-text');
                    let multiLineItems = $('#cmb-content-wrapper').find('.cmb-editable-text-container');
                    singleLineItems.each(function (){
                        $(this).attr('data-cmb-lang-id',cmbCountedExistingLangElements);
                        currentLangData[cmbCountedExistingLangElements] = $(this).html();
                        cmbCountedExistingLangElements++;
                    })
                    multiLineItems.each(function (){
                        $(this).attr('data-cmb-lang-id',cmbCountedExistingLangElements);
                        currentLangData[cmbCountedExistingLangElements] = $(this).html();
                        cmbCountedExistingLangElements++;
                    })
