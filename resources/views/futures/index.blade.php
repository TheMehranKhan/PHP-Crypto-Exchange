...
@section('content')
    ...
    <div id="app">
        ...
        <p-dialog header="Close Position" v-model:visible="showClosePositionDialog" :modal="true"
                  :style="{width: '100%' , maxWidth:'512px'}">
            <div class="child w-100 trade-spot-form-controller" style="background: transparent !important">
                <span>Close Percent : @{{closePostionDetails.size}}%</span>
                <div class="slider-controller ">
                    <vue-slider
                            :class="`x-slider`"
                            v-model="closePostionDetails.size"
                            :marks="marks"
                            :tooltip="'active'"
                            :interval="5"
                            :hide-label="true"
                ...
                <p-button label="Close Position" style="height: 40px;" @click="onClosePosition()"
                          class="p-button-danger fw-b f-18"></p-button>

            </div>
        </p-dialog>

        <p-dialog header="Edit Tp/Sl" v-model:visible="editPosition.dialog" :modal="true"
                  :style="{width: '100%' , maxWidth:'420px'}">
            <div class="update-tp-sl-form" v-if="editPosition?.data">
                <div class="head">
                    <div class="controller">
                        <div class="child flex-row">
                            <span class="position-type bg-green">LONG</span>
                            <div class="d-flex flex-column align-items-start">
                                <span class="f-18">@{{editPosition?.data?.symbol ?? getOldSymbol}}</span>
                                <span class="title">crossMargin @{{editPosition?.data?.leverage}}X</span>
                            </div>
                        </div>
                        <div class="child ">
                            <span class="title">Market Price (USDT)</span>
                            <span class="f-16 green">--</span>
                        </div>
                    </div>
                    <div class="controller mb-3">
                        <div class="child">
                            <span class="title">Amount </span>
                            <span>@{{Currency(editPosition?.data?.size)}}</span>
                        </div>
                        <div class="child">
                            <span class="title">pnl</span>
                            <span>@{{editPosition?.data?.pnl}} %</span>
                        </div>
                        <div class="child">
                            <span class="title">Liq Price(USDT)</span>
                            <span>---</span>
                        </div>
                    </div>
                </div>
                <div class="panel-controller">
                    <div class="header">
                        <span class=" b-green">Take-Profit</span>
                        <p-button class="bg-green" @click="setTpSl()" :loading="loading"
                                  :disabled="editPosition.tp == 0 || !editPosition.tp" label="confirm"></p-button>
                    </div>
                    <div class="controller">
                        <label for="amount" class="f-14 mb-1">Stop Price</label>
                        <div class="amount-box">
                            <div class="amount-child">
                                <p-inputnumber :min-fraction-digits="1" :max-fraction-digits="8"
                                               v-model="editPosition.tp"
                                               id="amount" class="block w-100"
                                               input-style="border:.15rem solid #b5b5b5 !important;"></p-inputnumber>
                            </div>
                            <div class="amount-child">
                                <span class='pl-2 gary'>USDT</span>
                            </div>
                        </div>

                        <ul>
                            <li><p>When the Mark Price Reaches -- market Orders Will Be triggered And all positions will
                                    be
                                    Systematically Liquidated at the best Price.</p></li>
                        </ul>
                    </div>
                </div>
                <div class="panel-controller border-0">
                    <div class="header">
                        <span class=" b-red">Stop-Loss</span>
                        <p-button class="bg-red" @click="setTpSl()" :loading="loading"
                                  :disabled="editPosition.sl == 0 || !editPosition.sl" label="confirm"></p-button>
                    </div>
                    <div class="controller">
                        <label for="amount" class="f-14 mb-1">Stop Price</label>
                        <div class="amount-box">
            ...
            <div class="trade-topBar-component">
                ...
                <p-overlaypanel ref="op" appendTo="body" :showCloseIcon="false" id="overlay_panel"
                    ...
                    <div class="spot-coins-controller">
                            <span class="p-input-icon-left">
                                ...
                                <p-inputtext type="text" v-model="filters['global'].value"
                        ...
                        <template v-if="wsAssets.marketList.length > 0">
                            <p-datatable @row-select="onRowSelect" v-model:filters="filters"
                                         ...
                                         :value="wsAssets.marketList.filter(val => val.is_futures )"
                                         :loading="wsAssets.loading" scrollable
                                         scroll-height="500px">
                ...
                <div class="controller gap-1 align-center">
                    <template v-if="selected?.trade_coin">
                        ...
                        <div class="child gap-01 ">
                            <span class="fw-b gray f-13">{{__('30 D Change')}}</span>
                            <span class="f-14 fw-b " v-if="selected?.change_rate_30d"
                                  :class="(Number(selected?.change_rate_30d) > 0 ) ? 'green':'red'">@{{Currency(selected?.change_rate_30d)}}%</span>
                            <span class="f-14" v-else>---</span>
                        </div>
                        <div class="child gap-01">
                            <span class="fw-b gray f-13">{{__('Circulation')}}</span>
                            <span class="f-13 fw-n"
                                  v-if="selected?.circulation">@{{Currency(selected?.circulation)}}</span>
                            <span class="f-14" v-else>---</span>
                        </div>
                        <div class="child gap-01">
                            <span class="fw-b gray f-13">{{__('Circulation USD Rank')}}</span>
                            <span class="f-13 fw-n" v-if="selected?.circulation_usd_rank">@{{Currency(selected?.circulation_usd_rank)}}</span>
                            <span class="f-14" v-else>---</span>
                        </div>
                        <div class="child gap-01">
                            <span class="fw-b gray f-13">{{__('Volume')}}</span>
                            <span class="f-13 fw-n"
                                  v-if="selected?.volume_usd">@{{Currency(selected?.volume_usd)}}</span>
                            <span class="f-14" v-else>---</span>
                        </div>
                    </template>
                ...
                <div class="controller align-center ml-auto moreinfo">
                    ...
                    <a href="{{route('exchange.index')}}">
                        <p-button label="{{__('Spot')}}" href="#" icon="pi pi-chart-bar f-16"
                                  class="p-button-text p-button-plain f-14 light-text px-1"></p-button>
                    </a>
            ...
            <div class="row mx-0 w-100 mt-1 ">
                <div class="child px-1 col-lg-10">
                        ...
                        <div class="col-lg-3 pl-1 pr-0">
                            <div class="order-book-screen-controller">
                                ...
                                <template v-if="orderBookSelectedTab == 0">
                                    ...
                                    <div class="order-book-controller">
                                        <div class="controller w-100 between px-2">
                                            <span class="gray">{{__('Price')}}(USDT)</span>
                                            <span class="gray">{{__('Total')}}(@{{selected?.trade_coin ?? getOldSymbol}})</span>
                                        </div>
                                        ...
                                        <template v-if="(sortType == 1 || sortType == 3)">
                                            <div v-if="!!orderBooks && !!orderBooks?.ask"
                                                 ...
                                                 @click="()=> orderPrice = ask[0]">
                                                <template v-if="!!ask[0]">
                                                    <div class="line"
                                                         :style="{width:Math.round(Number(ask[2]))+'%'}"></div>
                                        ...
                                        <template v-if="!!orderBooks && !!orderBooks?.bid">
                                            <template v-if="(sortType == 1 || sortType == 2)">
                                                <div v-for="bid in orderBooks?.bid.slice((sortType == 1 ? orderBooks?.bid?.length == 20 ? 13 : 3 :  orderBooks?.bid?.length == 20 ? 6 : 0))"
                                                     :key="bid[0]" class="order-line buy cp"
                                                     @click="()=> orderPrice = bid[0]">
                                                    <template v-if="!!bid[0]">
                                                        <div class="line"
                                ...
                                <div class="order-book-controller "
                                    ...
                                    <div class="controller w-100 between px-2">
                                        <span class="gray">{{__('Price')}}(USDT)</span>
                                        <span class="gray">{{__('Amount')}} (@{{selected?.trade_coin ?? getOldSymbol}})</span>
                                    </div>

                                    <div v-if="!!recountTrade " v-for="(item , index) in recountTrade" :key="index"
                                         class="order-line cp">
                                        <span class="flex-2 j-start ">@{{ Currency(item?.price) }}</span>
                                        <span class="flex-2 j-end fw-b"
                                              :class="item.order_type == 'buy' ? 'green':'red'">
                                    ...
                                    <div class="child mt-3 gap-1" v-if="recountTrade.length == 0">
                                        <img src="{{ asset('images/websocket/dark-empty.png') }}" width="150"
                                             class="mx-auto">
                                        <span class="text-center w-100 d-block p-3 gray f-17 fw-b">Trade Not Founded !</span>
                                    </div>
                                    <p-skeleton v-else v-for="item in 5" :key="item" width="100%"
                                                height="29px"></p-skeleton>
                                </div>
                    ...
                    <div class="history-screen-controller">
                        <div class="controller w-100 between align-center">
                            <div class="tab-controller">
                                <div v-for="(tab , index) in historyTabs" :key="index" class="tab"
                                     :class="historySelectedTab == index && 'active'"
                                     @click="historySelectedTab = index">
                                    <span>@{{ tab }}</span>
                                </div>
                            </div>
                        ...
                        <div class="mt-3 w-100">
                            ...
                            @endguest
                            @auth
                                <div class="panel-current" v-for="(item , index) in (positions.filter((item)=> {
                                if (selectedHistorySubTab == 0 && !item.stop_limit) return true;
                                    ...
                                    <div class="head">
                                        <div class="side">
                                            <div class="symbol">
                                                <span :class="`f-20 green`">@{{item?.symbol}} LONG</span>
                                                <div class="column-controller">
                                                    <div :class="`colum ${Number(45) < -80 ? 'red': 'green'}`"></div>
                                                    <div :class="`colum ${Number(45) < -40 ? 'red': 'green'}`"></div>
                                                    <div :class="`colum ${Number(45) < 0 ? '': 'green'}`"></div>
                                                    <div :class="`colum ${Number(45) > 40 ? 'green': ''}`"></div>
                                                    <div :class="`colum ${Number(45) > 80 ? 'green': ''}`"></div>
                                                </div>
                                            </div>
                                            <div class="margin-type">
                                                <span>crossMargin  @{{item?.leverage}}X</span>
                                            </div>
                                        </div>
                                        <div class="side">
                                            <div class="position-details">
                                                <p-button label="Close Position" @click="()=> {
                                            closePostionDetails = {
                                                size:100,
                                                id:item?.id
                                            };
                                    ...
                                    <div class="current-details">
                                        <div class="child">
                                            <span>{{__('Symbol')}}</span>
                                            <span class="green f-17 fw-b"> @{{Currency(item?.size)}} USDT</span>
                                        </div>
                                        <div class="child">
                                            <span>{{__('Realize')}}</span>
                                            <span>@{{Currency(item?.realised_cost)}}</span>
                                        </div>
                                        <div class="child">
                                            <span>{{__('PNL %')}}</span>
                                            <span :class="Number(item?.pnl) > 0 ? 'green':'red'" class="fw-b">@{{item?.pnl}} %</span>
                                        </div>
                                        <div class="child">
                                            <span>{{__('Avg. Entry')}}</span>
                                            <span>@{{Currency(item?.avg_entry_price ?? 0)}} USDT </span>
                                        </div>
                                        <div class="child">
                                            <span>{{__('Liq. Price')}}</span>
                                            <span>@{{Currency(item?.liq ?? 0)}}</span>
                                        </div>
                                        <div class="child">
                                            <span>{{__('Market Price')}}</span>
                                            <span>--</span>
                                        </div>
                                        <div class="child">
                                            <span>{{__('Position Margin')}}</span>
                                            <span>@{{Currency(item?.pos_margin ?? 0)}}</span>
                                        </div>
                                        <div class="child">
                                            <span>{{__('Position value')}}</span>
                                            <span>--</span>
                                        </div>
                                        <div class="child">
                                            <span>{{__('Margin Rate')}}</span>
                                            <span>@{{Currency(item?.maint_margin ?? 0)}}</span>
                                        </div>
                                        <div class="child">
                                        <span>TP/SL <i class="pi pi-pencil cp f-15 ml-2 cp" @click="()=> {
                                            editPosition = {
                                                dialog:true,
                                                id:item?.id,
                                                tp:item?.tp ?? null,
                                            ...
                                            <span>@{{item?.tp ? Currency(item?.tp) : '--'}}/@{{item?.sl ? Currency(item?.sl) : '--'}}</span>
                                        </div>
                                    </div>
                                </div>

                                <div v-if="positions.length == 0" class="empty-box-controller">
                                    <img src="{{ asset('images/websocket/dark-empty.png') }}">
                                    <h5 class="gary"> {{__('Positions Not Found')}}</h5>
                                </div>
                                {{-- <div class="panel-current" >
                                    <div class="head">
                                        <div class="side">
                                            <div class="symbol">
                                                <span :class="`f-20 red`">BTCUSDT SHORT</span>
                                                <div class="column-controller">
                                                    <div :class="`colum red`"></div>
                                                    <div :class="`colum red`"></div>
                                                    <div :class="`colum ${Number(-45) < 0 ? '': 'green'}`"></div>
                                                    <div :class="`colum ${Number(-45) > 40 ? 'green': ''}`"></div>
                                                    <div :class="`colum ${Number(-45) > 80 ? 'green': ''}`"></div>
                                            ...
                                            <div class="margin-type">
                                                <span>crossMargin  5X</span>
                                            </div>
                                        </div>
                                        <div class="side">
                                        <div class="position-details">
                                            <p-button label="Close Position" />
                                        </div>
                                        </div>
                                    </div>
                                    <div class="current-details">
                                        <div class="child">
                                            <span>Amount</span>
                                            <span class="red f-17 fw-b"> 3,2500 USDT</span>
                                        </div>
                                        <div class="child">
                                            <span>Realize</span>
                                            <span>12,312</span>
                                        ...
                                        <div class="child">
                                            <span>PNL %</span>
                                            <span :class="Number(-10) > 0 ? 'green':'red'" class="fw-b">15 %</span>
                                        </div>
                                        <div class="child">
                                            <span>Avg. Entry</span>
                                            <span >20,210 USDT </span>
                                        </div>
                                        <div class="child">
                                            <span>Liq. Price</span>
                                            <span>5,311</span>
                                        ...
                                        <div class="child">
                                            <span>TP/SL <i class="pi pi-pencil cp f-15 ml-2" ></i></span>
                                            <span>24,100/21,000</span>
                                        </div>
                                    </div>
                                </div> --}}
                            @endauth
                ...
                <div class="col-lg-2 px-0">
                    ...
                    <div class="trade-spot-form-controller">
                        <div class="controller w-100 between align-center">
                            <span class="f-16 fw-b">{{__('Future')}}</span>
                        ...
                        <div class="child gap-08">
                            <div class="controller flex-row between ">
                                ...
                                <span class="f-13"> @{{userBalance?.base_coin_balance ? Currency((isBuy ? (Number(userBalance?.base_coin_balance) - buyFloatingBalance)  : (Number(userBalance?.trade_coin_balance) - buyFloatingBalance) )) : '--'}} @{{(!selected?.trade_coin || isBuy) ?  'USDT' : selected?.trade_coin ?? 'USDT'}} </span>
                            </div>
                            ...
                            <div class="p-inputgroup">
                                <span class="p-inputgroup-addon">
                                    <span class="f-13 fw-b"> {{__('Leverage')}}</span>
                                </span>
                                <p-dropdown panelClass="pt-2 f-16" @change="calc()" v-model="levrage" class="ml-auto"
                                            :options="leverageList" option-label="label"
                                            option-value="value"></p-dropdown>
                            </div>
                            <p class="red f-13 mb-0" v-if="levrage > 3 && levrage < 5">{{__('Low Risk')}}</p>
                            <p class="red f-13 mb-0"
                               v-else-if="levrage >= 5 && levrage < 10">{{__('Moderate Risk')}}</p>
                            <p class="red f-13 mb-0" v-else-if="levrage >= 10">{{__('High Risk')}}</p>
                        </div>
                        <div class="child">
                            <div class="slider-controller ">
                                <vue-slider
                        ...
                        <div class="child w-100">
                            <div class="p-inputgroup">
                                <p-inputnumber disabled v-model="orderValue"
                                               placeholder="{{__('Order Value')}}"></p-inputnumber>
                                <span class="p-inputgroup-addon">
                        ...
                        <p-selectbutton v-model="futuresType" :options="['Cross','isolate']"
                                        aria-labelledby="single"></p-selectbutton>

                        @guest
                            <p-button class="trade-submit-btn">
                                <div class="controller flex-row both-center w-100 f-16">
                                    <span>{{ __('Please') }}</span>
                                    <a href="{{ route('register.index') }}"><span
                                                class="gold">{{ __('Register') }}</span></a>
                                    <span>{{ __('or') }}</span>
                                    <a href="{{ route('login') }}"><span class="gold">{{ __('Log In') }}</span></a>
                                </div>
                            </p-button>
                            ...
                            <div class="flex-1 px-1">
                                <div class="controller py-1 px-2 w-100 normal-radius gradient-left-green  between">
                                    <div class="child center">
                                        ...
                                        <span class="green fw-b f-13">@{{calcAmount?.asks > 0 ? Currency(calcAmount?.asks) : '--'}}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="flex-1 px-1">
                                <div class="controller py-1 px-2 w-100 normal-radius gradient-left-red  between">
                                    <div class="child center">
                                        ...
                                        <span class="red fw-b f-13">@{{calcAmount?.asks > 0 ? Currency(calcAmount?.bids) : '--'}}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
