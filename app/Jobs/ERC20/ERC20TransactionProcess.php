        ...
        if (empty($wallet)) {
            ...
            if (empty($deposit)) {
                $depositData = [
                    ...
                    'type' => TRANSACTION_DEPOSIT,
                    'status' => $this->transaction['status'] ? STATUS_COMPLETED : STATUS_PENDING,
                    'api' => API_ERC20,
                ];
                $depositService->deposit($depositData);
...
//                $this->sendToSystemWallet($wallet);
                ...
                if (!empty($deposit) && $deposit->status == STATUS_PENDING && $this->transaction['status']) {
                    $depositData = [
                        'wallet_id' => $wallet->id,
                        'symbol' => $wallet->symbol,
                        'address' => $this->transaction['recipient'],
    ...
    private function sendToSystemWallet($wallet)
    {
        ...
        if (bccomp($allowance, intval($this->transaction['amount'])) < 0) {
