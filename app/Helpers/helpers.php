...
if (!function_exists('get_nav_json')) {
    function get_nav_json($slug)
    {
        return app(NavigationService::class)->navigationJson($slug, "json");
    }
}
...
if (!function_exists('active_side_nav')) {
    function active_side_nav()
    {
        return auth()->check() && (auth()->user()->assigned_role === USER_ROLE_ADMIN
                || (auth()->user()->assigned_role === USER_ROLE_EDITOR)
                || (auth()->user()->assigned_role === USER_ROLE_OPERATOR)
                || (auth()->user()->assigned_role === USER_ROLE_CMS));
    }
}
...
if (!function_exists('is_light_mode')) {
    function is_light_mode($active, $inactive = '')
    {
        ...
        return settings('layout_mode_dark') ? $inactive : $active;
    }
}
    ...
    function short_code($string)
    {
        ...
        if (preg_match_all($pattern, $string, $matches)) {
            foreach ($matches[0] as $key => $value) {
                ...
                $output = preg_replace('/\>(.*?)\</i', '>' . $output . '<', $value);
...
if (!function_exists('dynamic_language')) {
    function dynamic_language($string, $fallbackLang, $languageData = [], $returnArray = false)
    {
        $outputArray = [];

        if (empty($languageData)) {
            $languageData = $fallbackLang;
        }
        $string = preg_replace('/\s\s+/', ' ', $string);
        $pattern = '/\<span[^\>]+data-cmb-lang-id="(.*?)\>(.*?)\<\/span\>/i';

        _dynamicLanguageProcessor($pattern, $fallbackLang, $languageData, $string, $outputArray);
        $pattern = '/\<div[^\>]+data-cmb-lang-id="(.*?)\>(.*?)\<\/div\>/i';

        _dynamicLanguageProcessor($pattern, $fallbackLang, $languageData, $string, $outputArray);

        if ($returnArray) {
            return [
                'objectData' => (object)$outputArray,
                'text' => $string
            ];
        }
        return $string;
    ...
    function _dynamicLanguageProcessor($pattern, $fallbackLang, $languageData, &$string, &$outputArray)
    {

        if (preg_match_all($pattern, $string, $matches)) {
            foreach ($matches[0] as $key => $value) {
                $subPattern = '/data-cmb-lang-id="[^"]+"/i';
                preg_match($subPattern, $value, $match);
                ...
                $dataString = preg_replace(['/data-cmb-lang-id="/', '/"/'], ['', ''], $match[0]);

                if (!$dataString || (!isset($languageData[$dataString]) && !isset($fallbackLang[$dataString]))) {
                    continue;
                }
                $dataStringOutput = isset($languageData[$dataString]) && $languageData[$dataString] !== '' ? $languageData[$dataString] : (isset($fallbackLang[$dataString]) ? $fallbackLang[$dataString] : '');

                $outputArray[$dataString] = $dataStringOutput;

                $output = preg_replace('/\>(.*?)\</i', '>' . $dataStringOutput . '<', $value);
                $string = str_replace($value, $output, $string);
            }
        }
    }
...
if (!function_exists('available_lang')) {
    function available_lang($directory): array
    {
        $path = 'page_language/' . $directory . '/';
        $allFiles = [];
        $files = Storage::disk('public')->files($path);
        foreach ($files as $file) {
            $fileName = str_replace([$path, '.json'], '', $file);
    ...
    if (!function_exists('get_decimal')) {
        function get_decimal($symbol)
        {
            $coinData = Coin::select('decimal_place')->where('symbol',$symbol)->first();
            return !empty($coinData->decimal_place) ? (int)$coinData->decimal_place : 8;
        }
    }

    if (!function_exists('send_sms')) {
        function send_sms($phoneNumber, $message, $type = 'SMS', $template = 'verify', $token)
        {
            if (settings('sms_provider_service') == ACTIVE) {
                try{
                    $receptor = $phoneNumber;
                    $token = !empty($token[0]) ? $token[0] : '';
                    $token2 = !empty($token[1]) ? $token[1] : '';
                    $token3 =!empty($token[2]) ? $token[2] : '';
...
//                    dd($e->errorMessage());
                ...
                try {
                    $sid = settings('twilo_sid'); // Your Account SID from www.twilio.com/console
                    $token = settings('twilo_auth'); // Your Auth Token from www.twilio.com/console
                    $client = new Twilio\Rest\Client($sid, $token);
                    if ($type == "SMS") {
                        $message = $client->messages->create(
                            $phoneNumber, // Text this number
                            [
                                'from' => settings('twilo_sender'), // From a valid Twilio number
                                'body' => $message
                            ]
                        );
                    } else {
                        $call = $client->account->calls->create(
                            ...
                            settings('twilo_sender'), // Valid Twilio phone number
                            array(
                                "record" => True,
    ...
    if (!function_exists('get_post_by_post_id')) {
        function get_post_by_post_id($id)
        {
            $post = Post::find($id);
            return $post;
        }
    ...
    if (!function_exists('get_bank_account_by_id')) {
        function get_bank_account_by_id($id)
        {
