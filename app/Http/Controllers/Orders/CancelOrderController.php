...
class CancelOrderController extends Controller
{
    public function destroy(Order $order)
    {
        try {
            ...
            if (request()->ajax()) {
                return response()
                    ->json([RESPONSE_MESSAGE_KEY => $exception->getMessage()], 200);
            } else {
                return redirect()
                    ->back()
        ...
        } elseif (isset(request()->provider) && request()->provider == 'nodejs'){
            return response()
                ->json([RESPONSE_TYPE_SUCCESS => true,RESPONSE_MESSAGE_KEY => $successMessage], 200);
        } else {
            return redirect()
                ->back()
