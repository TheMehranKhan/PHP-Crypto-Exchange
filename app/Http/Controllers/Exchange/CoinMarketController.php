...
class CoinMarketController extends Controller
{
    ...
    public function getCoinMarket($baseCoin): JsonResponse
    {
        ...
        $coinPairs = CoinPair::select('base_coin', 'trade_coin', 'name', 'last_price', 'is_futures')
        ...
        $coinPairs = $coinPairs->map(function ($coinPair) {
            $formattedCoinPair = [
                ...
                'trade_pair_symbol' => str_replace('_', '/', $coinPair->name),
                'is_futures' => $coinPair->is_futures,
            ];
    ...
    public function getAllCoinMarket(): JsonResponse
    {
        $baseCoins = cache()->rememberForever('baseCoins', function () {
            $baseCoins = CoinPair::where('is_active', ACTIVE)->pluck('base_coin');
