    ...
    public function store(StakeRequest $request, Wallet $wallet)
    {
        if (count($wallet->coin->coinStakes) == 0) {
            return response()->json([
                ...
                RESPONSE_MESSAGE_KEY => __("The stake service is currently disabled. Please try sometime later.")
            ], 200);
        }
        ...
        if (bccomp($request->get('amount'), $wallet->primary_balance) > 0) {
            return response()->json([
                RESPONSE_STATUS_KEY => false,
                RESPONSE_MESSAGE_KEY => __("You don't have enough balance to stake!. Your current balance is :amount", [
        ...
        if (bccomp($stake_package->minimum_amount, $request->get('amount')) > 0) {
            return response()->json([
                RESPONSE_STATUS_KEY => false,
                RESPONSE_MESSAGE_KEY => __("Failed to stake! The Minimum amount for this package is :amount", [
                    'amount' => $stake_package->minimum_amount,
                ])
            ], 200);
        }
        ...
        try {
            ...
            return response()->json([
                RESPONSE_STATUS_KEY => false,
                RESPONSE_MESSAGE_KEY => __("Unable to stake amount.")
            ], 200);
        }
        ...
        return response()->json([
            RESPONSE_STATUS_KEY => false,
            RESPONSE_MESSAGE_KEY => __("Your stake has been placed successfully.")
        ], 200);
    }
