...
class BlogController extends Controller
{
    ...
    public function categoryPosts($slug): View
    {

        $data['title'] = __('Blog').' '.$slug;
        $data['posts'] = Post::where('is_published', ACTIVE)->where('category_slug',$slug)
            ->with('postCategory')
            ->withCount('comments')
            ->orderBy('created_at', 'desc')
            ->simplePaginate(12);
    ...
    {
        $data['recentPosts'] = Post::select(['id', 'title', 'slug','featured_image', 'created_at'])
            ->where('is_published', ACTIVE)
            ->orderBy('created_at', 'desc')
            ->take(5)
            ->get();
