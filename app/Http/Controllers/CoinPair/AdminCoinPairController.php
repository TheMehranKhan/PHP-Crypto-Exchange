...
class AdminCoinPairController extends Controller
{
    ...
    public function store(CoinPairRequest $request): RedirectResponse
    {
        try {
            DB::beginTransaction();
            $attributes = $request->only('trade_coin', 'base_coin', 'is_active', 'last_price', 'is_default', 'is_futures');
    ...
    {
        $attributes = $request->only('coin', 'base_coin', 'last_price', 'is_active', 'is_futures');
