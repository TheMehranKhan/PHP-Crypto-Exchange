...
class UserRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        if ($this->isMethod('POST') && settings('require_mobile_verification') == ACTIVE) {
            $rules["assigned_role"] = "required|exists:roles,slug";
            $rules["email"] = "required|email|unique:users,email|between:5,255";
            $rules["mobile"] = "required|unique:users,mobile|between:10,255";
        ...
        } else if($this->isMethod('POST') && settings('require_mobile_verification') == INACTIVE) {
            $rules["assigned_role"] = "required|exists:roles,slug";
            $rules["email"] = "required|email|unique:users,email|between:5,255";
            $rules["username"] = "required|unique:users,username|max:255";
            $rules["is_email_verified"] = "required|in:" . array_to_string(verified_status());
    ...
    public function attributes()
    {
        return [
            ...
            'is_mobile_verified' => __('email status'),
