...
class FileUploadService
{
    public function upload($file, $filePath, $fileName, $prefix = '', $suffix = '', $disk = 'public', $width = null, $height = null, $fullView = true, $fileExtension = 'png', $quality = 100)
    {
        ...
        if (in_array($mimeType, $imageMimeTypes)) {
            ...
            if (!is_null($width) && !is_null($height) && $mimeType !== "image/svg+xml" && is_int($width) && is_int($height)) {
                if ($fullView) {
            ...
            } elseif (!is_null($width) && $mimeType !== "image/svg+xml" && is_int($width)) {
                $imageFile->resize($width, null, function ($constraint) {

                    $constraint->aspectRatio();

                });
            } elseif (!is_null($height) && $mimeType !== "image/svg+xml" && is_int($height)) {
