...
class ApplicationSettingService
{
    ...
    private function _password($key, $data_array, $input_class, $value_data, $place_holder, $input_start_tag, $input_end_tag)
    {
        $error = '';
        if (isset($this->errorMessages[$key])) {
            $error = '<span class="invalid-feedback">' . $this->errorMessages[$key][0] . '</span>';
        ...
        $output = $input_start_tag . '<input class="' . $input_class . '" type="password" value="' . $value_data . '" name="settings[' . $key . ']" placeholder="' . $place_holder . '">' . $input_end_tag;
